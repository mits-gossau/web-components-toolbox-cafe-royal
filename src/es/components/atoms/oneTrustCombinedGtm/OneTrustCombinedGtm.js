...
*/
export default class OneTrustCombinedGtm extends Shadow() {
  /**
   * @param {any} args
   */
  ...
  async callOptanonWrapper () {
    return this.loadScriptDependencyPromise || (this.loadScriptDependencyPromise = new Promise((resolve, reject) => {
      ...
      })(window, document, "script", "dataLayer", "${this.getAttribute('gtm-id') || 'GTM-XXXX'}")`
      const fn = 'function OptanonWrapper() {if (OptanonActiveGroups.includes(\'C0004\')){' + gtm + '}}'
      try {
